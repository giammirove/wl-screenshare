...
//
// Audio encoding - thanks to wlstream, a lot of the code/ideas are taken from
// there
...
bool check_fmt_available(const AVCodec *codec, AVSampleFormat fmt) {
  for (const enum AVSampleFormat *sample_ptr = codec->sample_fmts;
...
static enum AVSampleFormat convert_codec_sample_fmt(const AVCodec *codec,
                                                    std::string requested_fmt) {
  static enum AVSampleFormat converted_fmt =
      av_get_sample_fmt(requested_fmt.c_str());
  if (converted_fmt == AV_SAMPLE_FMT_NONE) {
    std::cerr << "Failed to find the given sample format: " << requested_fmt
              << std::endl;
    std::exit(-1);
  } else if (!codec->sample_fmts || check_fmt_available(codec, converted_fmt)) {
    std::cerr << "Using sample format " << av_get_sample_fmt_name(converted_fmt)
...
void FrameWriter::init_audio_stream() {
  ...
  const AVCodec *codec =
      avcodec_find_encoder_by_name(params.audio_codec.c_str());
  if (!codec) {
    std::cerr << "Failed to find the given audio codec: " << params.audio_codec
              << std::endl;
    std::exit(-1);
  }
  ...
  if (params.sample_fmt.size() == 0) {
    ...
    std::cerr << "Choosing sample format "
              << av_get_sample_fmt_name(audioCodecCtx->sample_fmt)
              << " for audio codec " << codec->name << std::endl;
  } else {
    audioCodecCtx->sample_fmt =
        convert_codec_sample_fmt(codec, params.sample_fmt);
  }
#if HAVE_CH_LAYOUT
  av_channel_layout_from_mask(&audioCodecCtx->ch_layout,
                              get_codec_channel_layout(codec));
#else
  audioCodecCtx->channel_layout = get_codec_channel_layout(codec);
  audioCodecCtx->channels =
      av_get_channel_layout_nb_channels(audioCodecCtx->channel_layout);
#endif
...
#else
  av_opt_set_channel_layout(swrCtx, "in_channel_layout", AV_CH_LAYOUT_STEREO,
                            0);
